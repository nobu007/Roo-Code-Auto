...
/**
 * VSCodeコマンドとして実行するためのラッパー
 */
export function registerGitHubLMTestCommands(context: vscode.ExtensionContext) {
	// 基本テストコマンド
	const basicTestCommand = vscode.commands.registerCommand("roo-code.testGitHubLM", async () => {
	...
	// 複数モデルテストコマンド
	const multiTestCommand = vscode.commands.registerCommand("roo-code.testMultipleGitHubModels", async () => {
	...
	// Roo Code設定テストコマンド
	const configTestCommand = vscode.commands.registerCommand("roo-code.testRooCodeGitHubConfig", async () => {
	...
	context.subscriptions.push(basicTestCommand, multiTestCommand, configTestCommand)
}

// 直接実行用（デバッグ時）
if (require.main === module) {
	console.log("This script should be run within VSCode extension context")
	console.log("Use the registered commands:")
	console.log("  - roo-code.testGitHubLM")
	console.log("  - roo-code.testMultipleGitHubModels")
	console.log("  - roo-code.testRooCodeGitHubConfig")
}
